...
const Test = () => {
    ...
    const playSoundTrack = (e) => {
        ...
        QuestionsData.forEach((item) => {
            ...
            else if(separatedStrings[0] == item.id && separatedStrings[1] == "audio2"){
                soundTrackToBePlayed = item.audio2
            }
            else if(separatedStrings[0] == item.id && separatedStrings[1] == "audio3"){
                soundTrackToBePlayed = item.audio3
            }
            else if(separatedStrings[0] == item.id && separatedStrings[1] == "audio4"){
    ...
    return (
        <div className="testContainer">
            <Navbar />
            <div className='timerDiv'><h2>{timer}</h2></div>
                ...
                >
                    {QuestionsData.map((item, index) => (
                        <SwiperSlide key={index} className='questionSlide'>
                            ...
                            <div className='questionImageCard'><img className='questionImage' src={QuestionsData[index].image} /></div>
                            <div className='optionsDiv'>
                                <div className='singleRadioButtonAndCard'>
                                    <input className='radioButtonInputCircle' type="radio" name="audio" checked={`selectedSet${QuestionsData[index].id}` === QuestionsData[index].audio1_name} value={QuestionsData[index].audio1_name} onChange={(e) => radioChangeHandler(e, QuestionsData[index].id)} />
                                    <div id={`${QuestionsData[index].id}_audio1`} className='singleOptionCard' onClick={playSoundTrack}><VolumeUpIcon id="volumeUpIconStyle" className='volumeUpIconStyle' /></div>
                                </div>
                                <div className='singleRadioButtonAndCard'>
                                    <input className='radioButtonInputCircle' type="radio" name="audio" checked={`selectedSet${QuestionsData[index].id}` === QuestionsData[index].audio2_name} value={QuestionsData[index].audio2_name} onChange={(e) => radioChangeHandler(e, QuestionsData[index].id)} />
                                    <div id={`${QuestionsData[index].id}_audio2`} className='singleOptionCard' onClick={playSoundTrack}><VolumeUpIcon id="volumeUpIconStyle" className='volumeUpIconStyle' /></div>
                                </div>
                                <div className='singleRadioButtonAndCard'>
                                    <input className='radioButtonInputCircle' type="radio" name="audio" checked={`selectedSet${QuestionsData[index].id}` === QuestionsData[index].audio3_name} value={QuestionsData[index].audio3_name} onChange={(e) => radioChangeHandler(e, QuestionsData[index].id)} />
                                    <div id={`${QuestionsData[index].id}_audio3`} className='singleOptionCard' onClick={playSoundTrack}><VolumeUpIcon id="volumeUpIconStyle" className='volumeUpIconStyle' /></div>
                                </div>
                                <div className='singleRadioButtonAndCard'>
                                    <input className='radioButtonInputCircle' type="radio" name="audio" checked={`selectedSet${QuestionsData[index].id}` === QuestionsData[index].audio4_name} value={QuestionsData[index].audio4_name} onChange={(e) => radioChangeHandler(e, QuestionsData[index].id)} />
                                    <div id={`${QuestionsData[index].id}_audio4`} className='singleOptionCard' onClick={playSoundTrack}><VolumeUpIcon id="volumeUpIconStyle" className='volumeUpIconStyle' /></div>
                                </div>
                            </div>
                            {
                                index === 9 ?
                                    <div>
                                        <button className='testSubmitButton' onClick={getAnswers}>Submit</button>
                                    </div> : none
                            }
